...
export class GeminiProvider implements CLIProvider {
    ...
    for (const { type, file, matcher } of hookFiles) {
      ...
      const entryIndex = existing.findIndex((h: HookEntry) =>
        h.hooks?.some((hh: { command?: string }) =>
          hh.command?.includes(`gemini/${file}`) ||
          hh.command?.includes(`gemini/${platformFile}`) ||
          hh.command?.includes(`gemini\\${platformFile}`)
        )
      );
  ...
  buildScheduledScript(params: {
    ...
    const dorothy = path.join(params.homeDir, '.dorothy');

    return generateScript({
      ...params,
      taskId: '',
      bashCommand: `"${bp}" --output-format stream-json --debug --include-directories "${dorothy}" -p '${escapeBashArg(params.prompt)}'`,
      cmdCommand:  `${qCmd(bp)} --output-format stream-json --debug --include-directories ${qCmd(dorothy)} -p "${escapeCmdArg(params.prompt)}"`,
    });
  }
}
